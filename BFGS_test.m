...
Z_resamp   = 100;
maxIter    = 4e4;   % Max number of iterations
trainIter  = 1;     % Number of Posterior updates per new sample
trainSteps = 100;
numDescent = 10;     % Number of fmincon iterations
resamp = 1;

%Learning Rates:
alpha = 1;      % Update posterior
kappa = 1;      % Prediction
beta  = 0.01;      % Hyperparameters
beta_min   = 0.001;
beta_reduc = 1;%0.9999
...
%options.Hessian = 'off'; options.Method = 'sqp'; active-set |
%interior-point | interior-point-convex | levenberg-marquardt | ...
                           %sqp | trust-region-dogleg |
                           %trust-region-reflective
%options.TolFun  = 1e-3; options.UseParallel = true
...
for ii = Z:numIter
    ...
    for i = 1:Q
        ...
        if param_update == true && iter >= 1 && mod(iter,trainSteps) == 0
            ...
            a = BFGS(hyp_opt,5, 0.0001, 1,0.5 ,20,...
                sn2(i),x,y,Z,D,RAND,n);
            hyp(i,:) = a(1:end-1);
...
%%
% figure(555),clf(555) hold on plot(t,Y(i_plot,1))
% plot(t,muPred','LineWidth',2) plot(t,muPred'+sqrt(s2'),'k')
% plot(t,muPred'-sqrt(s2'),'k') hold off
...
%     figure(150),clf(150) hold on plot(t,Y(i_plot)')
%     plot(t,muPred(i_plot-Z+1)','--k')
%     plot(t,muPredSp(i_plot-Z+1)',':m','LineWidth',1.5)
%     legend('Measured','pred. w. Meas.','pred. w. Set.') xlabel('time
%     (s)') ylabel('Current (mA)') hold off
end
...
%MAE = errorMeasure(Y_m,Y_p,'method','MAE','show',true); MSE =
%errorMeasure(Y_m,Y_p,'method','MSE','show',true);
...
%MAE = errorMeasure(Y_m,Y_psp,'method','MAE','show',true); MSE =
%errorMeasure(Y_m,Y_psp,'method','MSE','show',true);
...
plot(t,Y(i_plot,i)','k','LineWidth',1.5);               % Measured signal
plot(t,muPred(i_plot-Z+1,i)','--b','LineWidth',1.5);    % Predictions from
measurements plot(t,muPredSp(i_plot-Z+1,i)',':r','LineWidth',1.5);   %
Predictions from setpoint %Plot switch if onlyPredict < numIter
plot([onlyPredict onlyPredict]*ts, [min(Y(i_plot-1,i))
max(Y(i_plot-1,i))],':k','LineWidth',1.5) end
legend('Measured','\mu_{measure}','\mu_{setpoint}','\sigma_{measure}','\sigma_{setpoint}');
xlabel('time (s)') ylabel('Current (mA)') hold off

ax2 = subplot(2,1,2); hold on
plot(t(1:trainSteps:end),hyp,'LineWidth',1.5); hyp_mean(i,:) =
mean(hyp(round(length(hyp)*fm):end,:)); plot(t([1,end]),[hyp_mean(i,:);
hyp_mean(i,:)],'--k'); hold off xlabel('time (s)'); legend('$l_1$
$(\theta)$','$l_2$ $(\dot{\theta})$','$l_3$
$(\ddot{\theta})$','$\sigma_s$','Interpreter','Latex');
set(gcf,'Color','w') hold off linkaxes([ax1,ax2],'x');
%}
...
ax81 = subplot(3,1,1); hold on plot(t,X(i_plot,3)')
plot(t,Xsp(i_plot,3)','m') legend('Measured','Setpoint') ylabel('normalized
acc.') xlim([10.4 10.8])

hold off

ax82 = subplot(3,1,2); hold on plot(t,Y(i_plot)') plot(t,muPred','--k')
plot(t,muPredSp',':m','LineWidth',1.5) legend('Measured','pred. w.
Meas.','pred. w. Set.') ylabel('Current (mA)') xlim([10.4 10.8]) hold off

mu_muPredSp  = mean(muPredSp); sig_muPredSp = std(muPredSp); muPredSp =
(muPredSp - mu_muPredSp) ./ sig_muPredSp;

ax83 = subplot(3,1,3); hold on plot(t,muPredSp') plot(t,muPredSp' -
Xsp(i_plot,3)'*(1)) %plot(t,muPredSp'-Xsp(i_plot,2)'*(1002/2.4521))
xlim([10.4 10.8]) hold off
%}
