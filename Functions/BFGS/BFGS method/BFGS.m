...
% BFGS([0.5 0.5],20, 0.0001, 0, 1 ,20)
%
function ReturnValue = BFGS(dvar0,niter,tol,lowbound,intvl,ntrials,...
    varargin)
...
fs(1) = nlml(x,varargin{:}); % value of function at start
as(1)=0;
grad = (gradfunction(x,varargin{:})); % steepest descent
...
for i = 1:niter-1
    ...
    output = GoldSection_nVar(tol,x, ...
        s,lowbound,intvl,ntrials,varargin{:});
...
%     end
    ...
    delx = (x - xs(i,:))';
    gradold = gradfunction(xs(i,:),varargin{:});
