/* solve_upperTriangular - solve a linear system A*X = B with A being a
...
extern integer dtrtrs_(char *, char *, char *, integer *, integer *, doublereal *, integer *,
                                            doublereal *, integer *, integer *);  

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{                                        /* Matlab call: X = solve_upperTriangular(A, B) */
  doublereal *C;
  integer n, m, q;
  ...
  dtrtrs("U", "N", "N", &n, &m, mxGetPr(prhs[0]), &n, C, &n, &q); /* solve system */
  
  if (q < 0) mexErrMsgTxt("Error: illegal input to solve_upperTriangular");
}
