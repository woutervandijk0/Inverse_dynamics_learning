/* solve_lowerTriangular - solve a linear system A*X = B with A being a
...
{                                        /* Matlab call: X = solve_lowerTriangular(A, B) */
  ...
  dtrtrs("L", "N", "N", &n, &m, mxGetPr(prhs[0]), &n, C, &n, &q); /* solve system */
  
  if (q < 0) mexErrMsgTxt("Error: illegal input to solve_lowerTriangular");
}
