/* File    : solve_lowerTriangMatrix_sfun.c
 * Abstract:
 *
 * Solve a linear system A*X = B with A being a
 * triangular matrix using LAPACK/DPOTRS.
 *
 *
...
#define S_FUNCTION_NAME  solve_lowerTriangMatrix_sfun
...
#define MDL_CHECK_PARAMETERS
  ...
  {
      /* Check 1st parameter: Matrix size parameter */
      if (mxGetPr(NFEATURE_PARAM(S))[0] <= 0){
		ssSetErrorStatus(S,"Negative matrix dimensions not allowed!");
      }
  }
 ...
 *  -  Set dimension of inputport  1 to [D x D]
 *  -  Set dimension of outputport 0 to [D x D]
 */
static void mdlInitializeSizes(SimStruct *S)
{
	...
	ssSetInputPortMatrixDimensions(S, 1, D, D);				/* Set dimensions */
    ssSetInputPortRequiredContiguous(S, 1, true); /*direct input signal access*/
    ssSetInputPortDirectFeedThrough(S, 1, 1);
	
	if (!ssSetNumOutputPorts(S, NOUTPUTS)) return;
	/* X: a dynamically sized 1-D matrix output */
    ssSetOutputPortMatrixDimensions(S, 0, D, D);			/* Set dimensions */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	dtrtrs("L", "N", "N", &n, &m, A, &n, X, &n, &info); /* solve system */
	
	if (info < 0) mexErrMsgTxt("Error: illegal input to solve_lowerTriangular");
}
